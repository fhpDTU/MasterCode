...
vectors_true(:,1) = cos(angles)*(sx_true-cx_true)+sin(angles)*(sy_true+cy_true)+cx_true; 
vectors_true(:,2) = sin(angles)*(sx_true-cx_true)-cos(angles)*(sy_true+cy_true)+cy_true;  
vectors_true(:,3) = cos(angles)*(dx_true-cx_true)+sin(angles)*(cy_true-dy_true)+cx_true;  
vectors_true(:,4) = sin(angles)*(dx_true-cx_true)+cos(angles)*(dy_true-cy_true)+cy_true;  
vectors_true(:,5) = cos(angles + t_true/180*pi)*detector_width/p; 
vectors_true(:,6) = sin(angles + t_true/180*pi)*detector_width/p;
...
vectors(:,1) = cos(angles)*(sx0-cx0)+sin(angles)*(sy0+cy0)+cx0; 
vectors(:,2) = sin(angles)*(sx0-cx0)-cos(angles)*(sy0+cy0)+cy0;  
vectors(:,3) = cos(angles)*(dx0-cx0)+sin(angles)*(cy0-dy0)+cx0;  
vectors(:,4) = sin(angles)*(dx0-cx0)+cos(angles)*(dy0-cy0)+cy0;  
vectors(:,5) = cos(angles + t0/180*pi)*detector_width/p; 
vectors(:,6) = sin(angles + t0/180*pi)*detector_width/p;
